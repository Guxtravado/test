(async()=>{
  try{
    const cm=document.cookie.match(/platform_account_id=(\d+)/);
    if(!cm){console.error('[AI] cookie platform_account_id nao encontrado');return}
    const a=+cm[1];

    const c=window.Pulse?.getCsrfToken?.()
    ...
    let fid=11313294;
    try{
      const fr=await fetch('/apps_edge/platform-app-features/monday',{credentials:'include'});
      if(fr.ok){
    ...
    const gr=await fetch('/ai-agent/agent-instances/account/'+a+'/feature/'+fid,{
    ...
    const r=await fetch('/ai-agent/chats/object-level-chats/account/'+a+'/with-initial-messages/stream',{
